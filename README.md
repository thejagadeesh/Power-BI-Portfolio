# Power BI Data Analytics Portfolio | Business Intelligence Dashboard Projects
Welcome to my Power BI Data Analytics Portfolio. This repository showcases end-to-end Power BI dashboard projects where I transform raw data into interactive visualizations and actionable business insights for data-driven decision-making.

![--](https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.png)


## 📁 How to View My Work
Each project is organized into its own dedicated folder to keep the technical documentation clear and accessible.

* **Explore Folders:** Click on the project titles below to see detailed case studies, insights, and logic.
* **Technical Deep-Dive:** Download the `.pbix` files from each folder to view the dashboards, DAX measures, and data models in **Power BI Desktop**.

![--](https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.png)


## Featured Projects

### 1. [Sales Performance & Revenue Analytics (GUVI)](./sales-revenue-multi-currency-analysis)
**Focus:** Revenue Growth & Marketing ROI
* **Key Metrics:** 13.77M (INR) / $163.51K (USD) Total Sales | 19.04% Conversion.
* **Technical Highlights:** Multi-currency reporting toggle (INR/USD), Sales Funnel velocity (Lead-to-Sale), and Refund analysis.
* **Business Impact:** Identified that 80% of users are in "Pending Payment" status, uncovering a massive revenue recovery opportunity.
* **[View Project Folder ↗](sales-revenue-performance-analysis)**

### 2. [HR Analytics: Workforce Presence](./workforce-presence-analysis)
**Focus:** Operational Efficiency & Hybrid Work
* **Key Metrics:** 91.83% Presence | 10% WFH Preference.
* **Technical Highlights:** Daily attendance tracking, Time Intelligence DAX, and Workplace behavior modeling.
* **Business Impact:** Provided data-backed evidence to optimize office utility costs by identifying Fridays as the highest WFH days.
* **[View Project Folder ↗](workforce_presence_analysis)**

### 3. [HR Analytics: Compensation & Equity](./hr-compensation-demographics-analysis)
**Focus:** Strategic HR & Pay Parity
* **Key Metrics:** $54K Average Salary | 161 Total Headcount.
* **Technical Highlights:** Gender pay gap analysis, Education-to-Salary correlation (Treemaps), and Cumulative growth trends.
* **Business Impact:** Confirmed strong gender pay equity across the organization and calculated the ROI of employee upskilling.
* **[View Project Folder ↗](hr-compensation-demographics-analysis)**

![--](https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.png)

## 🛠️ Technical Stack

- Power BI: Built interactive dashboards and visual reports  
- DAX: Created measures for KPIs and business calculations  
- Power Query: Performed data cleaning and transformation (ETL)  
- Data Modeling: Established relationships and structured the dataset  

![--](https://raw.githubusercontent.com/andreasbm/readme/master/assets/lines/rainbow.png)

## Connect with me 🤝
I'm excited to connect with data enthusiasts and professionals! ⚡Let's explore the world of data together and create meaningful 🪄insights! 

📬Email: thejagadeeshr@gmail.com

[![linkedin](https://img.shields.io/badge/linkedin-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/thejagadeeshr/)
[![Kaggle](https://img.shields.io/badge/Kaggle-1DA1F2?style=for-the-badge&logo=Kaggle&logoColor=white)](https://kaggle.com/dinocojagadeeshr)
[![Tableau](https://img.shields.io/badge/Tableau-032d60?style=for-the-badge&logo=tableau&logoColor=white)](https://public.tableau.com/app/profile/dinoco/vizzes)
[![credly](https://img.shields.io/badge/credly-ff6a00?style=for-the-badge&logo=credly&logoColor=white)](https://www.credly.com/users/jagadeeshr/badges)

Thank you for visiting my profile, and don't forget to check out 🚀 [![My Portfolio](https://img.shields.io/badge/My%20Portfolio-ffffff?style=for-the-badge&logo=google-earth&logoColor=0041A5&color=ffffff&labelColor=ffffff)](https://sites.google.com/view/jagadeeshr)

<img src="https://raw.githubusercontent.com/BEPb/BEPb/194bc176c0b3f2ef01a883ff206499b86c5ce72f/assets/Bottom_down.svg" width="4000" height="250" />
<h4 align="center">
<h2 align="center">
  <a href="https://sites.google.com/view/jagadeeshr" title="Portfolio"> Show More 🔍</a>
</h2>

